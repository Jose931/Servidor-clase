...
public class ServletControlador extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (todo == null) {
			...
			if (LibrosBD.getStock(idLibro) > cantidad) {
				...
				if (elCarrito == null) { // el carrito est´a vac´ıo
					...
					Iterator it = elCarrito.iterator();
					while (!encontrado && it.hasNext()) {

						ElementoPedido unElementoPedido = (ElementoPedido) it.next();
...
//				String sqlUpdate = "UPDATE libros set cantidad = " + (LibrosBD.getStock(item.getIdLibro()) - item.getCantidad())
//						+ "WHERE autor LIKE " + ;
				float precio = LibrosBD.getStock(item.getIdLibro());
